...
from skrf.vi.vna.cmt.cobalt import (
    Cobalt,
    TraceParameter,
    TriggerScope,
    TriggerSource,
...
def test_channel_frequency_set_get(analyzer):
    ch = analyzer.ch1
    start = 1e6
    stop = 20e9
    npoints = 101
...
def test_freq_step_property(analyzer):
    ch = analyzer.ch1
    ch.freq_start = 1e6
    ch.freq_stop = 20e9
    ch.npoints = 401
...
def test_alloate_channels(analyzer):
...
def test_active_channel(analyzer):
    num_ch = 2
    assert isinstance(analyzer.active_channel, skrf.vi.vna.Channel)
